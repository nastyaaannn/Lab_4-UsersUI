...
fun TrashScreen(viewModel: MainViewModel) {
    ...
    val selectedNotes: List<NoteModel> by viewModel.notesInTrash
        .observeAsState(listOf())
    ...
    Scaffold(
        ...
        content = {
            Content(
                notes = notesInThrash,
                onNoteClick = { viewModel.onNoteSelected(listOf(it)) },
            ...
            if (dialog != NO_DIALOG) {
                val confirmAction: () -> Unit = when (dialog) {
                    ...
                    PERMANENTLY_DELETE_DIALOG -> {
                        {
                            viewModel.onNoteSelected(selectedNotes)
                            dialogState.value = NO_DIALOG
                        }
                    }
                    else -> {
...
private fun ViewModel.onNoteSelected(note: List<NoteModel>) = Unit
    ...
    Column {
        ...
        LazyColumn {
            items(count = filteredNotes.size) { noteIndex ->
                ...
                val isNoteSelected = selectedNotes.contains(note).also {
                    Note(
                        note = note,
